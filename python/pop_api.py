...
def get_personhoodscore(w3, network, address):
    ...
    # prevent char limit issues with rinkeby
    # + validate network
    if network == 'rinkeby':
        try:
    ...
    if balance == 0:
        return 0
    index = contract.functions.tokenOfOwnerByIndex(address, 0).call()
    url = contract.functions.tokenURI(index).call()

    # for local testing
    # url = url.replace('https://persons.proofofpersonhood.com', 'http://localhost:8000')

    # get data from json
    try:
        response = requests.get(url).json()
        for attr in response['attributes']:
            if attr['trait_type'] == 'personhood_score':
                return attr['value']
    except:
        pass
    return 0